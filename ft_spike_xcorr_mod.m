function [stat] = ft_spike_xcorr_mod(cfg, spike)
...
cfg = ft_checkopt(cfg, 'method', 'char', {'xcorr', 'shiftpredictor', 'jitter'});
...
if strcmp(cfg.method,'jitter')
  s = cell(nChans,nChans);
else 
...
% make sure indx(1) <= indx(2)
cmbindx = sort(cmbindx,2);
...
for iTrial = 1:nTrials
  ...
  for iCmb = 1:nCmbs
    ...
    switch cfg.method
      ...
      case 'jitter'
        % according to Buzaki's method
        if ~exist('jitterCell','var')
          jitterCell = cell(nTrials,nChans);
        end
        if ~isempty(ts1) && ~isempty(ts2)
          if isempty(jitterCell{iTrial,chansel == indx(2)})
            jt = 0.01*rand(1000,length(ts2)) - 0.005;
            jitterCell{iTrial,chansel == indx(2)} = jt;
          else
            jt = jitterCell{iTrial,chansel == indx(2)};
          end
          [x]   = spike_crossx_jitter(ts1(:),ts2(:)'+jt,cfg.binsize,nLags*2+1);  % removed the mex file for now, until issue on windows is resolved
          % remove the center peaks from the auto-correlogram
          if indx(1)==indx(2), x(nLags:nLags+1,:) = 0; end
          % sum the xcorr
          if isempty(s{indx(1),indx(2)})
            s{indx(1),indx(2)} = x;
          else
            s{indx(1),indx(2)} = s{indx(1),indx(2)} + x;
          end
          if isempty(s{indx(2),indx(1)})
            s{indx(2),indx(1)} = flip(x);
          else
            s{indx(2),indx(1)} = s{indx(2),indx(1)} + flip(x);
          end      
        end      
...
function [Cj] = spike_crossx_jitter(X,Y,binsize,nbins)
  % lighter and quicker version using matrix operation
  % X is original spike train of cell A (length-m vector)
  % Y is independently jittered spike trains of cell B (1000*n matrix)
  m=length(X);
  n=size(Y,2);
  X = sort(X(:));
  Y = sort(Y,2);
  ...
  j = 0:nbins-1;
  B = minLag + j * binsize;
  % rearrange matrix shape for multiplication
  X = X(:);
  Y = Y(:)';
  Y=repmat(Y,m,1);
  ...
  % compute all distances at once using a multiplication trick
  D = log(exp(-X).*exp(Y));
  D = mat2cell(D,m*ones(1,1000),n);
  Cj = cellfun(@(x) histc(x(abs(x)<=abs(minLag)),B),D,'UniformOutput',false)';
  Cj = cellfun(@(x) sum([x(:),isempty(x)*zeros(nbins,1)],2),Cj,'UniformOutput',false);
  Cj = cell2mat(Cj);
  Cj = Cj(1:end-1,:);
