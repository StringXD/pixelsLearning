...
if true
    ...
    conncount=nan(105,6,2);
    pairscount=nan(105,6);
    for bin=1:6
    ...
    for bin=1:6
        for ses=1:105
            ...
            conncount(ses,bin,1)=nnz(diff(reg_chains_S1{bin}(selS1,:),1,2) == 0);
            conncount(ses,bin,2)=nnz(diff(reg_chains_S2{bin}(selS2,:),1,2) == 0);
            pairsel=pair_chains{bin}(:,1)>ses*100000 & pair_chains{bin}(:,1)<(ses+1)*100000;
            pairscount(ses,bin)=nnz(diff(pair_regs{bin}(pairsel,:),1,2) == 0);
        end
    end
    mn=[];
    for i=1:105
...
congru_active_count=nan(105,6,2);
incong_active_count=nan(105,6,2);
congru_inactive_count=nan(105,6,2);
incong_inactive_count=nan(105,6,2);

congru_active_pair=nan(105,6);
incong_active_pair=nan(105,6);
congru_inactive_pair=nan(105,6);
incong_inactive_pair=nan(105,6);
...
errorbar(1:5,[mmn,mmis,mmia,mmcs,mmca],[stdn/sqrt(105),stdis/sqrt(105),stdia/sqrt(105),stdcs/sqrt(105),stdca/sqrt(105)],'k.','CapSize',12)
xlim([0.5,5.5])
ylim([0,0.25]);
...
saveas(gcf,'connDensity5BarsLocal.fig');
