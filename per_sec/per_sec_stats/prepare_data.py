...
def prepare_data(delay=6,debug=False):
    ...
    for path in sorted(futil.traverse(dpath)):
        print(path)
        with h5py.File(os.path.join(path, "FR_All.hdf5"), "r") as ffr: # Trial-aligned firing rate
            # print(list(ffr.keys()))
            ...
            trial_FR = np.array(ffr["FR_All_1000"], dtype="double")
        ...
        folder_list.extend([path] * SU_ids.shape[1]) # (n(SU),)
