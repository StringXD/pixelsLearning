...
def judgePerformance(trials, criteria=75):
    ...
    return (inWindow)
        ...
        #        return ("learning", 2, np.where(inWindow == True), correctResp)
...
def matchDepth(depth, depthL, date, mice_id, imec_no, unlabeledRecord=None):
    if not depthL.empty:
        label = depthL.loc[
            (np.array(depthL["distance2tipLow"]) <= depth) & (depth < np.array(depthL["distance2tipHigh"])),
            ["acronym"]
        ]
...
def getRegionList(cmp=False):
    #site_file = r"/home/xd/data/learning/NP tracks revised.csv"
    site_file = r"E:\prJ\neuropixels\learning\NP tracks revised.csv"
...
if __name__ == "__main__":
    identical_counter = []
    regionL = getRegionList(cmp=False)
    unlabeledRecord = []
    ...
    for path in traverse(r"D:\neupix\DataSum"):
        ...
        for one_su in su_ids:
            ...
            if reg == reg_cmp:
                identical_counter.append([1, reg])
            else:
                identical_counter.append([0, reg])
